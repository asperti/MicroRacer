...
class Get_critic(tf.keras.Model):
    ...
    def call(self, inputs):
        ...
        #dist = tfp.distributions.Normal(mu, sigma)
        #value = dist.sample()  
        value =  mu + sigma * tfp.distributions.Normal(0,1).sample(num_actions)       
        #value = tf.clip_by_value(value, clip_value_min=mu- clip_boundary, clip_value_max=mu+ clip_boundary)
        return value
...
def update_critics(states, actions, rewards, dones, newstates):
    ...
    tcritic_v = target_critic([newstates, new_policy_actions])                    
...
def update_actor(states):
    ...
    with tf.GradientTape(watch_accessed_variables=False) as tape:
        ...
        critic_v = critic_model([states, new_policy_actions])  
